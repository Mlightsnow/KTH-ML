...
def viz_rf(weights,it,grid,tp='rf'):
    ...
    for x in range(grid[0]):
        for y in range(grid[1]):
            ...
            if y < 5:
                ...
                t = 20 + y
            # print(t+grid[0]/2*x)
            axs[x,y].imshow(weights[:,:,t+grid[0]*x], cmap="bwr", vmin=-imax, vmax=imax, interpolation=None)
    if tp == 'RECON':
      plt.savefig(f"./reconstructions/{tp}.iter%06d.png"%it)
    else:
        plt.savefig(f"./receptive_fields/{tp}.iter%06d.png"%it)
    plt.close('all')
