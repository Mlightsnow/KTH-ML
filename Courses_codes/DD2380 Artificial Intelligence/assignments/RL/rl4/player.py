...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        # It is recommended you use the np.random module
        epsilon = (epsilon_final - epsilon_initial) * current_total_steps / anneal_timesteps + epsilon_initial
        r = np.random.rand()
        if r > epsilon:
            action = np.nanargmax(Q[state])
        else:
    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max and diff > self.threshold:
            ...
            while not end_episode:
                ...
                #action = epsilon_greedy(Q, s_current, list_pos, eps_type="constant")

                action = epsilon_greedy(Q, s_current, list_pos, steps_total, self.epsilon_initial, 
                ...
                Q[s_current][action] += lr * (discount * Q[s_next][np.nanargmax(Q[s_next])] + R - Q[s_current][action])
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
            ...
            diff = abs(np.nanmean(Q - Q_old, dtype=np.float64))
            #print(Q - Q_old)
            #print(diff)
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
            Q_old[:] = Q
